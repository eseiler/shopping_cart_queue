...
public:
    ...
    void enqueue(slot_id slot, value_type value)
    {
...

        {
            ...
            {
                _empty_cart_queue_empty_or_closed_cv.wait(cart_management_lock,
                                                          [this, &slot_cart]
                                                          {
                                                              // wait until either an empty cart is ready, or the slot has a cart, or the queue was closed
                                                              return !_empty_carts_queue.empty() || !slot_cart.empty()
                                                                  || _queue_closed == true;
                                                          });
    ...
    cart_future_type dequeue()
    {
...

        {
            ...
            _full_cart_queue_empty_or_closed_cv.wait(full_cart_queue_lock,
                                                     [this]
                                                     {
...
struct slotted_cart_queue<value_t>::empty_carts_queue_t
{
    ...
    {
        assert(0 <= _count);
        assert(_count <= static_cast<std::ptrdiff_t>(_cart_count));

        if (!(0 <= _count))
            throw std::runtime_error{"empty_carts_queue.count: negative"};

        if (!(_count <= static_cast<std::ptrdiff_t>(_cart_count)))
            throw std::runtime_error{std::string{"empty_carts_queue.count: FULL, _count: "} + std::to_string(_count)
...
struct slotted_cart_queue<value_t>::full_carts_queue_t
{
    ...
    full_carts_queue_t(cart_count cart_count) : _count{0}, _cart_count{cart_count.cart_count}
    {
    ...
    void _check_invariant()
    {
        ...
        if (!(_count <= static_cast<std::ptrdiff_t>(_cart_count)))
            throw std::runtime_error{std::string{"full_carts_queue.count: FULL, _count: "} + std::to_string(_count)
                                     + " <= " + std::to_string(_cart_count)};
    }
